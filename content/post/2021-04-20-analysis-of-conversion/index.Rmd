...
Based on the drawing, we see that the experimental change in conversion lies much to the right than the change of conversion due to random processes. Thus, we can conclude that the change in conversion is 1.67% - is a statistically significant.
...
Let's simulate a random process, in which in 50357 outcomes conversion was in 13984 cases. That is, create a vector of length 50357, in which the 36373 elements are zero, and 13984 are equal to 1. Now many times (set, 1000) will randomly divide the sample into two parts: the first length 27182 and the second 23175.

In each part, we will consider the ratio of the number of units to the sample size and find the difference between themselves.

This conversion change we will save and, in the future, use to build a histogram.
